...
resource "null_resource" "ukrail_locations_upload" {
  provisioner "local-exec" {
    command = "java -cp lib/CSVFileAvroUploader-1.0.0.jar io.confluent.bootcamp.CSVFileAvroUploader -c ${var.ccloud-properties} -f ${var.ukrail_locations} --topic ${confluent_kafka_topic.LOCATIONS_RAW.topic_name} --key-field location_id"
  }
